layui.define(['element', 'oneplatform'], function (exports) {
    "use strict";
    var $ = layui.jquery,
    ...
    Navbar.prototype.render = function () {
        ...
        if (typeof (_config.elem) !== 'string' && typeof (_config.elem) !== 'object') {
        	oneplatform.error('Navbar error: elem参数未定义或设置出错，具体设置格式请参考文档API.');
        }
        var $container;
        ...
        if ($container.length === 0) {
        	oneplatform.error('Navbar error:找不到elem参数配置的容器，请检查.');
        }
        if (_config.data === undefined && _config.url === undefined) {
            oneplatform.error('Navbar error:请为Navbar配置数据源.')
        }
            ...
            if (_config.cached) {
                ...
                if (cacheNavbar.navbar === undefined) {
                    $.ajax({
                        ...
                        success: function (result, status, xhr) {
                        	...
                        	if(result.code!=200){layer.msg(result.msg, {icon: 5});return;}
                            //添加缓存
                        ...
                        error: function (xhr, status, error) {
                        	oneplatform.error('Navbar error:' + error +  "("+status+")");
                        },
                ...
                $.ajax({
                    ...
                    error: function (xhr, status, error) {
                    	...
                    	oneplatform.error('Navbar error:' + error);
                    },
                    complete: function (xhr, status) {
                        _that.config.elem = $container;
                    }
    ...
    Navbar.prototype.on = function (events, callback) {
        ...
        if (typeof (events) !== 'string') {
        	oneplatform.error('Navbar error:事件名配置出错，请参考API文档.');
        	return;
        }
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (data[i].children !== undefined && data[i].children !== null && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    ...
                    ulHtml += '<a href="javascript:;" data-url="' + data[i].children[j].code + '">';
                ...
                var dataUrl = (data[i].code !== undefined && data[i].code !== '') ? 'data-url="' + data[i].code + '"' : '';
