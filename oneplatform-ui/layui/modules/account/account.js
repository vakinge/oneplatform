  ...
  table.on('tool(table)', function(obj){ 
	  ...
	  if(layEvent === 'switch'){
	      layer.confirm('确认'+(data.enabled ? '禁用' : '启用' )+'么?', function(index){
		    	...
		    	jeesuitelayui.post('/api/account/switch',param,function(data){
			        layer.close(index);
	      ...
	      layer.confirm('确认删除么?', function(index){
	    	jeesuitelayui.post('/api/account/delete/'+data.id,null,function(data){
	    		obj.del(); 
		        layer.close(index);
	    	...
	    	jeesuitelayui.iframeDialog('编辑模块','/modules/account/edit.html?id='+data.id);
	    }
  });
