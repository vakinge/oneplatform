...
layui.define(['jeesuitelayui','laytpl', 'form', 'table'], function(exports){
	  ...
	  table.on('tool(dstable)', function(obj){ 
		  ...
		  if(layEvent === 'switch'){
		      ...
		      layer.confirm('确认删除么?', function(index){
		    	...
		    	jeesuitelayui.iframeDialog('编辑数据源','./edit.html?id='+data.id);
		    }
	  });
		  ...
		  table.on('tool(apitable)', function(obj){ 
			  ...
			  if(layEvent === 'switch'){
			      ...
			      layer.confirm('确认删除么?', function(index){
			    	...
			    	jeesuitelayui.iframeDialog('编辑接口','./edit.html?id='+data.id);
			    }else if(layEvent === 'details'){
			    	jeesuitelayui.iframeDialog('接口详情','./details.html?id='+data.id);
			    }
		  });
  ...
  $('#action_config_line').find('[action]').click(function(){
	  if(!dbaction){jeesuitelayui.error('请选择操作类型');return;}
	  if(!tableName){jeesuitelayui.error('请选择数据表');return;}
	  selectColumns = {};
  ...
  $('#add_action_btn').click(function(){
	  if(!tableName){jeesuitelayui.error('请先选择表名');return;}
	  var sql = $sqlEditer.val().trim();
	  if(sql === ''){jeesuitelayui.error('SQL表达式不能为空2');return;}
	  var actionData = {};
  ...
  $('#save_btn').click(function(){
	  if(apiActionList.length == 0){jeesuitelayui.error('请添加操作');return;}
	  var params = jeesuitelayui.serializeJson($('#add_form'));
  ...
  $('#test_api_btn').click(function(){
	  ...
	  if(method === 'GET'){
		  params = jeesuitelayui.serializeQueryParams($('#testapiForm'));
		  if(params !== '')uri = uri + '?' + params;
		  $.getJSON(uri,function(json){
			  if(json.code != 200){jeesuitelayui.error('执行错误:'+json.msg);return;}
			  var formatJson = jeesuitelayui.formatJson(JSON.stringify(json));
			  $('#response_previve').text(formatJson);
		  });
	  }else{
		  params = jeesuitelayui.serializeJson($('#testapiForm'));
		  $.ajax({
	        	dataType:"json",
			    type: "POST",
		        url: uri,
  ...
  function parseActionOptions(action){
	  ...
	  if(!tmp1){jeesuitelayui.error('请选择操作列');return false;}
