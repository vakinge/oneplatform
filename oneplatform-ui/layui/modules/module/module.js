...
layui.define(['jeesuitelayui', 'table'], function(exports){
  ...
  table.on('tool(table)', function(obj){ 
	  ...
	  if(layEvent === 'switch'){
	      layer.confirm('确认'+(data.enabled ? '禁用' : '启用' )+'么?', function(index){
		    	...
		    	jeesuitelayui.post('/api/module/switch',param,function(data){
			        layer.close(index);
	    	...
	    	jeesuitelayui.iframeDialog('API文档',data.apidocUrl);
	    }else if(layEvent === 'del'){
	      layer.confirm('确认删除么?', function(index){
	    	jeesuitelayui.post('/api/module/delete/'+data.id,null,function(data){
	    		obj.del(); 
		        layer.close(index);
	    	...
	    	jeesuitelayui.iframeDialog('编辑模块','/modules/module/edit.html?id='+data.id);
	    }
  });
