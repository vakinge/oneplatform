...
layui.define(['form','jeesuitelayui', 'layer', 'tree','element', 'table'], function(exports){
	  ...
	  if(nodeDatas){
		  layui.tree({
			    ...
			    ,click: function(item){ //点击节点回调
			    	...
			    	formdata.type = type;
			    	jeesuitelayui.dataRender($form,formdata);
		  ...
		  table.on('tool(table)', function(obj){ 
			  ...
			  if(layEvent === 'leave'){
			      layer.confirm('确认设置为离职状态么?', function(index){
				    	jeesuitelayui.post(API_BASE_PATH + '/employee/leave/'+data.id,null,function(data){
				    		obj.del(); 
					        layer.close(index);
					        jeesuitelayui.success('设置为离职状态成功');
				    	});
			      ...
			      layer.confirm('确认删除么?', function(index){
			    	jeesuitelayui.post(API_BASE_PATH + '/employee/delete/'+data.id,null,function(data){
			    		obj.del(); 
				        layer.close(index);
			    	...
			    	jeesuitelayui.iframeDialog('编辑员工','./edit.html?id='+data.id);
			    }
		  });
  }
  ...
  if(actionType === 'department-add'){
	  var companyId = jeesuitelayui.getQueryParams('companyId');
